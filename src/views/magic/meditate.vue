...
<script>
  ...
  export default {
    ...
    async created () {
      ...
      store.commit('help', 'txt_help_meditate')
      await this.$bindAsArray('buildings', database.ref('users').child(store.state.uid).child('constructions').orderByChild('name').equalTo('lbl_building_node'))
    }
  }
</script>
